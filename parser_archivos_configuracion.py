...
def main():
    try:
        ...
        # Resumen ejecutivo
        print("\nResumen Ejecutivo:")
        print(f"Se ha parseado el archivo de configuración {archivo_configuracion} con éxito.")
        print(f"El archivo contiene {len(configuracion)} secciones.")
        print(f"La primera sección tiene el valor {list(configuracion.values())[0]}.")
        
    except FileNotFoundError as e:
        print(f"Ocurrió un error: {str(e)}")
    except json.JSONDecodeError as e:
        print(f"Ocurrió un error al parsear el archivo de configuración: {str(e)}")
