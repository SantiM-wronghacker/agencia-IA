#!/usr/bin/env python3
# AREA: FINANZAS
# DESCRIPCION: Agente que realiza calculadora prima seguro
...
def calcular_prima(edad, monto_asegurado, tipo_vehiculo):
    try:
        ...
        if edad < 20 and tipo_vehiculo.lower() == "lujo":
            prima_anual *= 1.1
            prima_mensual *= 1.1
            prima_trimestral *= 1.1
            prima_semestral *= 1.1
...
def main():
    try:
        ...
        if resultado:
            ...
            print(f"Edad del asegurado: {edad}")
            print(f"Monto asegurado: ${monto_asegurado:.2f}")
            print(f"Tipo de vehículo: {tipo_vehiculo}")
            print(f"Factor edad: {resultado['factor_edad']:.2f}")
            print(f"Factor vehículo: {resultado['factor_vehiculo']:.2f}")
            print(f"Prima anual: ${resultado['prima_anual']:.2f}")
            print(f"Prima mensual: ${resultado['prima_mensual']:.2f}")
            print(f"Prima trimestral: ${resultado['prima_trimestral']:.2f}")
            print(f"Prima semestral: ${resultado['prima_semestral']:.2f}")
            print("Resumen ejecutivo:")
            print(f"El costo total del seguro anual es de ${resultado['prima_anual']:.2f}")
            print(f"El costo total del seguro mensual es de ${resultado['prima_mensual']:.2f}")
            print(f"El costo total del seguro trimestral es de ${resultado['prima_trimestral']:.2f}")
            print(f"El costo total del seguro semestral es de ${resultado['prima_semestral']:.2f}")
